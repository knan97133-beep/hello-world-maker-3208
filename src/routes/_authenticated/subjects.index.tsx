    ...
    <div className="space-y-6">
      ...
      {subjects.isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {list.map((s) => {
            const open = unlock.isUnlocked(s.year, s.semester);
            const passed = (unlock.scoreOf(s.id) ?? -1) >= 60;
            const body = (
              <>
                <div className="flex items-center justify-between gap-2">
                  <h2 className="flex items-center gap-2 font-bold group-hover:text-primary">
                    {!open && <Lock className="size-4 text-muted-foreground" />}
                    {passed && <CheckCircle2 className="size-4 text-primary" />}
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  {open
                    ? ar
                      ? `السنة ${s.year} · الفصل ${s.semester}`
                      : `Year ${s.year} · Semester ${s.semester}`
                    : ar
                      ? "مقفل — أنهِ اختبارات الفصل السابق بنجاح"
                      : "Locked — pass the previous term's quizzes"}
                </p>
              </>
            );
            ...
            return open ? (
              ...
              <div
                key={s.id}
                aria-disabled
                className="cursor-not-allowed rounded-2xl border border-dashed border-border/70 bg-muted/40 p-5 opacity-70"
              >
                {body}
              </div>
            );
